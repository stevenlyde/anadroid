...
import org.ucombinator.utils.{AIOptions, FancyOutput}
...
trait DSGAnalysisRunner extends DalvikVMRelated {
  ...
  private def joinNumEdges(curIndex: Int, nextIndex: Int, edgesTo: scala.collection.mutable.Map[Int, Set[Int]]) {
      ...
      nextStateIndexes.foreach {
        case nextIndx => {
          ...
          if ( //marked.contains(nextIndx) && 
            !currentPath.contains(nextIndx)) {
            //&& marked(nextIndx) == 0) {//!marked(nextIndx)) {
