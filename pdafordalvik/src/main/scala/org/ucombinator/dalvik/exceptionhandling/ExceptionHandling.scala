...
import org.ucombinator.dalvik.cfa.cesk.{CESKMachinary, StateSpace, StmtForEqual}
import org.ucombinator.dalvik.preanalysis.LiveRegisterAnalysis
import org.ucombinator.dalvik.statistics.Statistics
import org.ucombinator.dalvik.syntax.{DalvikClassDef, ExceptionHandlers, MoveExceptionStmt, RegisterExp, Stmt, StmtNil, ThrowStmt}
...
trait ExceptionHandling extends StateSpace with CESKMachinary with StmtForEqual with LiveRegisterAnalysis {
  ...
  def injectFaultStates(injThrownStmts: List[InjectThrownStmt], fp: FramePointer, s: Store, pst: PropertyStore,
                        kptr: KAddr, t: Time, st: Stmt, k: Kont): Set[Conf] = {
  ...
  def getInjectStatesFromAnnotations(handlerMgr: ExceptionHandlers, annotationExnVals: List[String], fp: FramePointer,
                                     s: Store, pst: PropertyStore, k: Kont, t: Time, st: Stmt, clsP: String,
                                     methP: String, kptr: KAddr,  nextLiveRegs: Set[String]): Set[Conf] = {
  ...
  private def getHandlerBranchStates(handlerMgr: ExceptionHandlers, exnAnnos: List[String], fp: FramePointer, s: Store,
                                     pst: PropertyStore, k: Kont, t: Time, st: Stmt, kptr: KAddr): Set[Conf] = {
  ...
  //private  
  def getInjectFaultStates(exnHandlers: ExceptionHandlers, exnAnnos: List[String], fp: FramePointer, s: Store,
                           pst: PropertyStore, k: Kont, t: Time, st: Stmt, clsP: String, methP: String, kptr: KAddr,
                           liveRegs: Set[String]): Set[Conf] = {
    //val tp = tick(List(st), t)
  ...
  private def genFaultStatesFromExnTypes(exnTypes: List[String], fp: FramePointer, s: Store, pst: PropertyStore,
                                         k: Kont, t: Time, st: Stmt, clsP: String, methP: String, kptr: KAddr,
  ...
  def injectAPIFaultStates(exnTypes: List[String], fp: FramePointer, s: Store, pst: PropertyStore, k: Kont, t: Time,
                           st: Stmt, clsP: String, methP: String, kptr: KAddr, liveRegs: Set[String]): Set[Conf] = {
    // val tp = tick(List(st), t)
    genFaultStatesFromExnTypes(exnTypes, fp, s, pst, k, t, st, clsP, methP, kptr, liveRegs)
  }

  def thrownUncaughtExnStates(objVals: Set[AbstractObjectValue], s: Store, pst: PropertyStore, fp: FramePointer,
                              kptr: KAddr, t: Time, clsP: String, methP: String, liveRegs: Set[String]): Set[Conf] = {
  ...
  def handleNormalThrownStmt(ps: PartialState, handlerType: String, handleExnType: String, lbl: String, ts: Stmt,
                             objValues: Set[Value], s: Store, pst: PropertyStore, nxt: Stmt, fp: FramePointer,
                             kptr: KAddr, t: Time, k: Kont): Set[Conf] = {
    ...
    objValues.foldLeft(Set[Conf]())((res, absV) => {
      ...
      res ++ //dealWithOnePossibleExnValue(ts, absV, s, fp, kptr, t, k,objValues)
        handleHandleFrame(ps, handlerType, handleExnType, lbl, clsName, s.mkDomainD(absV), k, fp, s, pst, kptr, t, ts)
    })
  }
  ...
  def handleInjectExnStmt(itS: InjectThrownStmt, s: Store, pst: PropertyStore, nxt: Stmt, fp: FramePointer, kptr: KAddr,
                          t: Time, k: Kont, clsP: String, methP: String): Set[Conf] = {
   ...
   */
  private def dealWithOnePossibleExnValue(ijt: Stmt, exnValue: Value, s: Store, pst: PropertyStore, fp: FramePointer,
                                          kptr: KAddr, t: Time, k: Kont, allAbsVals: D, clsP: String, methP: String): Set[Conf] = {
  ...
  def dealWithAllPossibleExnValues(ijt: Stmt, allExnVals: D, s: Store, pst: PropertyStore, fp: FramePointer,
                                   kptr: KAddr, t: Time, k: Kont, clsP: String, methP: String): Set[Conf] = {
   ...
   *
   */
  private def handleThrown(exnType: String, exnValue: D, k: Kont, fp: FramePointer, s: Store, pst: PropertyStore,
                           kptr: KAddr, t: Time, ijt: Stmt, allVals: D, clsP: String, methP: String): Set[Conf] = {
            ...
            handlerType match {
              ...
              case "finally" => {
                ...
                finallyHandlerStmtO match {
                  ...
                  case None => {
                    throw new CESKException("the finally handler is not inthe label table in injectThrown statement inspectStack")
                  }
                }
  ...
  def handleHandleFrame(ps: PartialState, handlerType: String, handleExnType: String, lbl: String, exnType: String,
                        exnValue: D, k: Kont, fp: FramePointer, s: Store, pst: PropertyStore, kptr: KAddr, t: Time,
                        ijt: Stmt): Set[Conf] = {
    val tp = tick(List(ijt), t)
    ...
    handlerType match {
      ...
      case "normal" => {
        ...
        if ((exnType == handleExnType) || DalvikClassDef.isInstanceofParents(exnType, handleExnType)) {
          ...
          //  if not found, it will continue to loop!
          val nxtN = ijt //getProperStmt(ijt, allVals)
