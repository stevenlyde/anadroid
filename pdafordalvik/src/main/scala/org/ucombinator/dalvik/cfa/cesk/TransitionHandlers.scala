...
import org.ucombinator.utils.{ForIntentFuzzerUtil, _}

/**
 * Part of the logic from StackCESK is extracted to this trait
 */
trait TransitionHandlers extends ExternalLibCallsHandler with ExceptionHandling with StmtForEqual {

  import org.ucombinator.domains.CommonAbstractDomains._

  private def resolveMethod(invokeType: String, oClsName: String, methPath: String, argTypes: List[String]): List[MethodDef] = {
    invokeType match {
      //direct
      case "direct" => {
  ...
  def handleNonStaticInvoke(invokeType: String, ivkS: AbstractInvokeStmt, methPath: String, realN: Stmt, objExp: AExp,
                            objAexp: AExp, argRegExps: List[AExp], ls: Stmt, fp: FramePointer, tyStrs: List[String],
                            tp: Time, s: Store, pst: PropertyStore, kptr: KAddr, t: Time, k: Kont,
                            stForEqual: StForEqual, pstate: PartialState): Set[Conf] = {
  ...
  def applyMethod(stForEqual: StForEqual, isEntryApply: Boolean, methBody: Stmt, regsNum: BigInt, ovO: Option[ObjectValue], fp: FramePointer,
    ...
    if (!isEntryApply) {
      ...
      // if it is entry apply,  no need to propagate the security property
   ...
   *
   */
  def applyAtomicOp(stForEqual: StForEqual, opCode: SName, opCodeType: String, lineNo: Stmt, destReg: RegisterExp,
                    aExps: List[AExp], s: Store, pst: PropertyStore, nxt: Stmt, fp: FramePointer, kptr: KAddr, tp: Time,
                    k: Kont): Set[Conf] = {
          ...
          if (isConstNum(opCode)) {
            if (aExps.length == 1) {
              aExps.head match {
                ...
                case _ => {
                  throw new CESKException("not IntExp in isConstNum" + opCode.toString() + lineNo.toString())
                }
              }
            }
          ...
          aExps.head match {
            case se@RegisterExp(_) => {
              ...
              if (noChange) {
                ...
                // should probably be a class name defined? or library class?
  ...
  def handleMove(destReg: RegisterExp, aExps: List[AExp], s: Store, pst: PropertyStore, nxt: Stmt, fp: FramePointer, kptr: KAddr, tp: Time, k: Kont): Set[Conf] = {
    ...
    val newState = (PartialState(buildStForEqual(nxt), fp, newStore, newPStore, kptr, tp), k)
    Set(newState)
  }
   ...
   */
  def fieldAssignHandleHelperS(isStaticGet: Boolean, fieldS: FieldAssignStmt, fieldExp: StaticFieldExp,
                               srcOrDestReg: RegisterExp, s: Store, pst: PropertyStore, nxt: Stmt, fp: FramePointer,
                               kptr: KAddr, tp: Time, k: Kont): Set[Conf] = {
  ...
  def fieldAssignHandleHelperI(isInstanceGet: Boolean, fieldAssignStmt: FieldAssignStmt, fieldExp: NonStaticFieldExp,
                               srcOrDestReg: RegisterExp, s: Store, pst: PropertyStore, nxt: Stmt, fp: FramePointer,
                               kptr: KAddr, tp: Time, k: Kont): Set[Conf] = {
    ...
    val objRegE = objRegExp match {
      ...
      case _ => {
        throw new SemanticException("the obj register in instance get statement is not RegisterExp" + objRegExp.toString())
      }
    }
