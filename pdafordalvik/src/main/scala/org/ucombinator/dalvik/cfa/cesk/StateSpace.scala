...
import org.ucombinator.dalvik.syntax.CommonSSymbols._
...
trait StateSpace {
  ...
  case class FramePointer(t: Time, meth: Stmt) extends Pointer {
     ...
     */
    def push(t: Time, invkSt: Stmt): FramePointer = {
 ...
 *//*
  ...
  *//**
  ...
  abstract sealed class ControlState {
    ...
    def getCurStore: Store = this match {
      case ps@PartialState(st, fp, s, pst, kptr, t) => s
      case FinalState(st) => st.mkEmptyStore
      case ErrorState(_, _, st) => st.mkEmptyStore
    }


    def getCurPropertyStore: PropertyStore = this match {
      case ps@PartialState(st, fp, s, pst, kptr, t) => pst
      case FinalState(st) => st.mkEmptyStore
      case ErrorState(_, _, st) => st.mkEmptyStore
    }
...
//    }
  ...
  def isMove(opCode: SName): Boolean = opCode match {
    case SMove | SMove16 | SMoveWide | SMoveWide16 | SMoveWideFrom16 | SMoveObject | SMoveObjectFrom16 | SMoveObject16 | SMoveFrom16 => true
    case _ => false
  }
  ...
  def mergeStores(initial: Store, newStores: List[Store]): Store = {
