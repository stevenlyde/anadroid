...
trait CESKMachinary extends StateSpace {
  ...
  def initObjectProperty(classPath: String, pst: PropertyStore, op: ObjectPointer, securityValues: D): PropertyStore = {
    val fieldPathStrs = getFieldTypeStrs(classPath)
    val fieldOffsets = fieldPathStrs.map {
      case (path, ftype) => op.offset(path)
    }
          ...
          v match {
            ...
            case av => {
              av match {
                ...
                case TrueValue() => {
                  Set(("bool", Set("true"))) ++ res2
                }
                case FalseValue() => {
                  res2 ++ Set(("bool", Set("false")))
                }
                case BoolTop => {
                  Set(("bool", Set("top"))) ++ res2
                }
