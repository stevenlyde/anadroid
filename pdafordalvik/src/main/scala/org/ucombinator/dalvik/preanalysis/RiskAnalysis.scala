...
import models.{JsonOutputForMethod, SourceLocationInfo}
import org.ucombinator.dalvik.syntax.{LineStmt, MethodDef, Stmt, _}
import org.ucombinator.playhelpers.AnalysisHelperThread
import org.ucombinator.utils.{AIOptions, CommonUtils, StringUtils}
import spray.json._
...
object RiskAnalysis {
  ...
  def getStmtInfo(st: Stmt): (String, String) = {
    st match {
      ...
      case iss@InvokeSuperStmt(_, argRegAExp, objAExp, tyStrs, nxt, ls, clsP, methP) => {
        iss.getClassPathAndMethodName
      }
      case iss@InvokeStaticStmt(_, _, _, _, _, _, _) => {
        iss.getClassPathAndMethodName
      }
      case is@InvokeStmt(_, _, _, _, _, _, _, _) => {
        is.getClassPathAndMethodName
      }
