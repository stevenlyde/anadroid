...
import org.ucombinator.dalvik.cfa.cesk.{CESKMachinary, StateSpace}
import org.ucombinator.dalvik.syntax.{AExp, RegisterExp, StForEqual, Stmt}
...
trait RawStringLibsAI extends StateSpace with CESKMachinary {
  ...
  def handleStringBuilderInit(invokS: Stmt, argRegExps: List[AExp], objValss: D, ls: Stmt, s: Store, pst: PropertyStore, realN: Stmt, fp: FramePointer, kptr: KAddr, t: Time, tp: Time, k: Kont): Set[Conf] = {
    val objVals = filterStrBuilderObjVals(objValss, s)
