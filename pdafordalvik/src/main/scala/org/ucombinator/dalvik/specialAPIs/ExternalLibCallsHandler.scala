...
import org.ucombinator.dalvik.syntax.{AExp, StForEqual, Stmt}
...
trait ExternalLibCallsHandler //extends StateSpace with
  ...
  def handleSpecialCalls(methPath: String, invokS: Stmt, argRegExps: List[AExp], objVals: D, ls: Stmt, s: Store,
                         pst: PropertyStore, realN: Stmt, fp: FramePointer, kptr: KAddr, t: Time, tp: Time,
                         k: Kont): Set[Conf] = {
    methPath match {
   ...
   */
  def propagateTaintPropertyForFunctionInvokeObjAndMoveResult(argAddrs: List[Addr], objAexps: List[AExp],
                                                              pst: PropertyStore, invokS: Stmt, nextStmt: Stmt,
                                                              clsP: String, methPath: String, lineNo: Stmt,
                                                              fp: FramePointer): PropertyStore = {
    ...
    if (anySensitiveVals.isEmpty)
      ...
      if (objAexps.isEmpty) {
        // static invoke, no need to affect the obejct, because no object be affected wtih

        if (isMoveResultS) {
          // the next is move result, pass the joined properties to the returned result
        ...
        if (isMoveResultS) {
          ...
          // will only affect the object security values then
  ...
  def handleExternalLibCalls(methPath: String, invokS: Stmt, argRegExps: List[AExp], objAexps: List[AExp], // static invoke will be empty
                             objVals: D, ls: Stmt, s: Store, pst: PropertyStore, realN: Stmt, fp: FramePointer,
                             kptr: KAddr, t: Time, tp: Time, k: Kont, stForEqual: StForEqual): Set[Conf] = {
