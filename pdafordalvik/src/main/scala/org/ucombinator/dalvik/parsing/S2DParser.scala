...
class S2DParser {
  ...
  private def parseClassDef(sx: SExp): Option[DalvikClassDef] = { //(fieldTable : ImmMap[SName, FieldDef], MethodTable: ImmMap[SName, MethodDef])
        ...
        hd match {
          case SField :+: attrs :+: rest => {
            ...
            if (attrList.length == 0) {
              // this is the case where the field does not have the acces flags in the SExp
  ...
  private def getRegLimits(sxl: SExp): (BigInt, List[SExp]) = {
    ...
    val hdLimit = sl.head
    hdLimit match {
      case SLimit :+: SRegs :+: (snum: SInt) :+: SNil() => {
        //(snum : SInt) => {
  ...
  private def parseThrowsAnnotation(sxl: SExp): (List[String], List[SExp]) = {
    ...
    if (sl.isEmpty) {
      ...
      val hdAnn = sl.head
      hdAnn match {
        case SAnnotation :+: SSystemVisibility :+: rest => {
          //(snum : SInt) => {
