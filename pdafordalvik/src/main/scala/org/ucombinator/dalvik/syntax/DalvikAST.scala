...
object StmtNil extends Stmt {
  ...
  def next_=(st: Stmt) {
    throw new ParsingExpcetion("can't change next on null stmt")
  }

  def lineNumber = throw new ParsingExpcetion("no line stmt")

  def lineNumber_=(st: Stmt) {
    throw new ParsingExpcetion("can't change linenumber on null stmt")
  }
    ...
    fe match {
      case nonStFE@NonStaticFieldExp(or, fp, ft) => {
        lhr match {
          ...
          case _ => {
            throw new Exception("@refRegsStrSet: cant be anything else")
          }
        }
      }
      //sget
...
case class InvokeStmt(
  ...
  override def toString = "InvokeStmt:" + methPathStr + "(" + argRegAExp + " , " + objAExp + " , " + tyStrs + ")" + StringUtils.stmtContextInfo(clsP, methP, lineNumber)
...
case class CatchStmt(typeStr: String, exnTy: String, fromLblStr: String, toLblStr: String, usingLblStr: String, nxt: Stmt, ls: Stmt, clsP: String, methP: String) extends Stmt {
  ...
  override def toString = "CatchStmt(" + typeStr + "," + exnTy + "," + fromLblStr + "," + toLblStr + ")" + StringUtils.stmtContextInfo(clsP, methP, lineNumber)
...
case class DalvikClassDef(
  ...
  def getSupersStr(res: List[String])(curClass: String): List[String] = {
    ...
    val curSuperClsDefOpt = DalvikClassDef.forName(curSuper)
    curSuperClsDefOpt match {
      // if not found, then also add the cursuper str
      case None => {
...
object DalvikClassDef {
    ...
    methClassDefO match {
      case Some(d) => {
        d.lookupCurClsInterfaceTableCache(methIndex) match {
          ...
          case None => {
            // go through the normal method resolution process
  ...
  def lookupMethod(clsName: String, methPath: String, argTypes: List[String], invokeType: Int): List[MethodDef] = {
    ...
    invokeType match {
      // static
      case 0 => {
        lookupStaticOrDirect(MethClassDef, methIndex)
      }
      // direct
      case 1 => {
        lookupStaticOrDirect(MethClassDef, methIndex)
